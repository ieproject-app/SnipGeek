...
heroImage: "https://res.cloudinary.com/snipgeek/image/upload/q_auto/f_auto/v1781144769/snipgeek/images/_posts/ubuntu/hide-grub-fix-plymouth/image-grub-changed.webp"
...
![File konfigurasi GRUB dengan semua pengaturan yang diterapkan.](https://res.cloudinary.com/snipgeek/image/upload/q_auto/f_auto/v1781144769/snipgeek/images/_posts/ubuntu/hide-grub-fix-plymouth/image-grub-changed.webp)
...
## Masalah 4: GRUB Masih Muncul Setelah Konfigurasi

Setelah menerapkan semua konfigurasi GRUB dan menjalankan `update-grub`, menu tetap muncul saat boot. Saya kaget — konfigurasinya sudah benar, tapi ada sesuatu yang menimpa setting saya.

Investigasi dengan `grep -i timeout /boot/grub/grub.cfg` mengungkap masalahnya: `set timeout=30` muncul di paling atas file, menimpa setting `hidden` di bawahnya. Penyebabnya adalah mekanisme failsafe GRUB — dia mendeteksi boot sebelumnya gagal (*recordfail*) dan memaksa menu tampil apa pun konfigurasinya.

Solusinya adalah menghapus flag recordfail dari environment GRUB:

```bash
sudo grub-editenv /boot/grub/grubenv list
```

Kalau kamu melihat `recordfail=1`, reset dengan:

```bash
sudo grub-editenv /boot/grub/grubenv unset recordfail
```

Setelah reboot, GRUB akhirnya skip menu seperti yang diinginkan.
