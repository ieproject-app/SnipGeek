...
export async function generateMetadata({
  ...
  const canonicalPath = `${currentPrefix}/category`;
  ...
  return {
    title: dictionary.categories.allCategoriesTitle,
    description: dictionary.categories.allCategoriesDescription,
    alternates: {
...
export default async function CategoriesPage({
  ...
  const breadcrumbSegments = [
    ...
    { label: dictionary.categories.allCategoriesTitle },
  ];
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            description: dictionary.categories.allCategoriesDescription,
            url: `https://snipgeek.com${linkPrefix}/category`,
          }),
        }}
      />
      <script
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16 sm:pb-24">
        <header className="mb-12 text-center">
          ...
          <h1 className="font-display text-4xl font-extrabold tracking-tighter text-primary mb-4 sm:text-5xl">
            {dictionary.categories.allCategoriesTitle}
          </h1>

          <p className="max-w-xl mx-auto text-muted-foreground text-sm sm:text-base leading-relaxed">
            {dictionary.categories.allCategoriesDescription}
          </p>
        </header>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.map((category) => {
            const style = getBadgeStyle(category.name);
            return (
              <Link
                key={category.slug}
                href={`${linkPrefix}/category/${encodeURIComponent(category.slug)}`}
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex flex-col gap-3">
                    ...
                    >
                      <FolderClosed className={cn("h-5 w-5", style.text)} />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex items-center gap-2 text-xs font-medium text-muted-foreground/60">
                        ...
                        <span className="capitalize">
                          {category.type === "both" ? "blog & notes" : category.type}
                        </span>
                      </div>
                    </div>
                  </div>
