...
export function Header({ searchableData, dictionary }: { searchableData: SearchableItem[], dictionary: Dictionary }) {
    ...
    const toggleTheme = () => {
        ...
        const applyThemeChange = () => {
            setTheme(nextTheme);
            if (msg) notify(msg, nextTheme === 'dark' ? <Moon className="h-4 w-4" /> : <Sun className="h-4 w-4" />);
        };
        ...
        <header
            ...
            <div className="max-w-4xl mx-auto h-16 min-h-[64px] px-4 md:px-6 flex items-center justify-between relative overflow-visible">
                ...
                <div className={cn(
                    ...
                    <NextLink href="/" className="flex items-center gap-2 sm:gap-3 group" aria-label="SnipGeek Home">
            ...
            >
                <div className="relative flex items-center justify-center gap-2.5 h-10 px-6 bg-background/90 backdrop-blur-xl border border-border/60 shadow-2xl rounded-full overflow-hidden pointer-events-auto ring-1 ring-black/[0.03]">
                    ...
                    <span className={cn(
                        ...
                        message ? "opacity-100 scale-100 rotate-0" : "opacity-0 scale-50 -rotate-45"
                    )}>
                        {icon && React.cloneElement(icon as React.ReactElement<any>, { className: "h-3.5 w-3.5" })}
                    </span>
