...
export function PostEditor({ initialData, id }: PostEditorProps) {
  ...
  const handleMediaUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!db || !files || files.length === 0) return;
    ...
    if (confirm(`Hapus permanen gambar ini dari server?`)) {
      try {
        ...
        if (imgToDelete.mediaId) {
          deleteDocumentNonBlocking(
            doc(db, "media_library", imgToDelete.mediaId),
          );
        }
        ...
        setFormData((prev) => ({
          ...
          images: newImages,
          heroImageUrl:
            prev.heroImageUrl === imgToDelete.url
              ? newImages[0]?.url || ""
              : prev.heroImageUrl,
        }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const currentCollection = formData.isPublished
      ? "blogPosts_published"
      : "blogPosts_drafts";
    const oldCollection = initialData?.isPublished
      ? "blogPosts_published"
      : "blogPosts_drafts";
    ...
    setDocumentNonBlocking(doc(db, currentCollection, docId), postData, {
      merge: true,
    });
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      heroImageAltText: e.target.value,
                    }))
                  }
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Windows, Tech, Hardware"
                  className="h-9"
                />
              </div>
            </CardContent>
