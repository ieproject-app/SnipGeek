...
import { Facebook, Linkedin, Send, MessageCircle, Link as LinkIcon, Check } from 'lucide-react';
...
export function ShareButtons({ title, imageUrl }: ShareButtonsProps) {
  ...
  const shareButtons = [
    ...
    },
    {
      ...
      onClick: handleCopy,
      hoverClass: copied ? "bg-emerald-500 text-white" : "hover:bg-emerald-500 hover:text-white",
        ...
        <div key={btn.id} className="flex items-center flex-1">
          {btn.href ? (
            <a 
              ...
              className={cn(
                "group relative flex flex-1 flex-col items-center justify-center py-4 px-2 rounded-full transition-all duration-300",
            ...
            >
              ...
              <btn.icon className={cn("h-5 w-5 transition-transform duration-300", btn.id === 'copy' && copied && "text-white")} />
            </button>
          )}
