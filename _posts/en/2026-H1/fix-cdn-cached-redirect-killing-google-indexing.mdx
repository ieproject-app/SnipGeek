...
<Steps>
  ...
  The core crawl rules were fine: `robots.txt` allowed `/` for all user agents, and `sitemap.xml` contained all 216 published URLs including every blog post. I later noticed one SEO hardening gap, though: `robots.txt` was also blocking `/_next/`, which is too aggressive for a Next.js site because Google may need those assets to render pages properly. I fixed that afterward.
...
- **Relaxed `robots.txt` for Next.js assets** — I stopped disallowing `/_next/` so crawlers can fetch the CSS/JS needed to render pages correctly.
